...
class WebUIServer:
    ...
    def _show_access_token(self):
        ...
        try:
            from astrbot.core.maibot.src.webui.token_manager import get_token_manager
    ...
    def _setup_anti_crawler(self):
        """配置防爬虫中间件"""
        try:
            from astrbot.core.maibot.src.webui.anti_crawler import AntiCrawlerMiddleware
    ...
    def _setup_robots_txt(self):
        """设置robots.txt路由"""
        try:
            from astrbot.core.maibot.src.webui.anti_crawler import (
    ...
    def _register_api_routes(self):
        ...
        try:
            # 导入所有 WebUI 路由
            from astrbot.core.maibot.src.webui.routes import router as webui_router
            from astrbot.core.maibot.src.webui.logs_ws import router as logs_router
            from astrbot.core.maibot.src.webui.knowledge_routes import (
                router as knowledge_router,
            )

            # 导入本地聊天室路由
            from astrbot.core.maibot.src.webui.chat_routes import router as chat_router

            # 导入规划器监控路由
            from astrbot.core.maibot.src.webui.api.planner import router as planner_router

            # 导入回复器监控路由
            from astrbot.core.maibot.src.webui.api.replier import router as replier_router
